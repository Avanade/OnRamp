...
{
    /// <summary>
    /// Provides the database capabilities.
    /// </summary>
    public static class Database
    {
        /// <summary>
        /// Gets all the table and column schema details from the database.
        /// </summary>
        /// <param name="dbConnection">The <see cref="DbConnection"/>.</param>
        /// <param name="excludeSqlServerSpecific">Indicates whether to skip the <i>Microsoft SQL Server</i> specific metadata queries.</param>
        /// <param name="additional">Provides an opportunity to perform additional configuration manipulation during the load.</param>
        public static async Task<List<DbTable>> GetSchemaAsync(DbConnection dbConnection, bool excludeSqlServerSpecific = false, Action<DbConnection, List<DbTable>>? additional = null)
        {
            ...
            await ExecuteDbStatementAsync(dbConnection, sr!, (dr) =>
            {
            ...
            foreach (var pks in ExecuteDatabaseStatementAsync(dbConnection, sr2!, (dr) =>
            {
                return new
                {
            ...
            if (!excludeSqlServerSpecific)
            {
                ...
                foreach (var fks in ExecuteDatabaseStatementAsync(dbConnection, sr3!, (dr) =>
                {
                    return new
                    {
                ...
                await ExecuteDbStatementAsync(dbConnection, sr4!, (dr) =>
                {
                ...
                await ExecuteDbStatementAsync(dbConnection, sr5!, (dr) =>
                {
                    var t = tables.Single(x => x.Schema == dr.GetValue<string>("TABLE_SCHEMA") && x.Name == dr.GetValue<string>("TABLE_NAME"));
                    var c = t.Columns.Single(x => x.Name == dr.GetValue<string>("COLUMN_NAME"));
                    t.Columns.Remove(c);
                ...
                await ExecuteDbStatementAsync(dbConnection, sr6!, (dr) =>
                {
                    var t = tables.Single(x => x.Schema == dr.GetValue<string>("TABLE_SCHEMA") && x.Name == dr.GetValue<string>("TABLE_NAME"));
                    var c = t.Columns.Single(x => x.Name == dr.GetValue<string>("COLUMN_NAME"));
                    c.IsComputed = true;
            ...
            // Perform final preparation and auto-determine foreign key relationships.
        ...
        /// <summary>
        /// Executes the database statement asynchronously.
        /// </summary>
        /// <param name="dbConnection">The <see cref="DbConnection"/>.</param>
        ...
        public static async Task ExecuteDbStatementAsync(DbConnection dbConnection, StreamReader statement, Action<DbDataReader> action)
        {
            if (dbConnection == null)
                throw new ArgumentNullException(nameof(dbConnection));
        ...
        /// <summary>
        /// Executes the database statement asynchronously returning a resultant collection.
        /// </summary>
        ...
        public static async Task<IEnumerable<T>> ExecuteDatabaseStatementAsync<T>(DbConnection dbConnection, StreamReader statement, Func<DbDataReader, T> func)
        {
            var list = new List<T>();
            await ExecuteDbStatementAsync(dbConnection, statement, (dr) => list.Add(func(dr)));
            return list;
        }
